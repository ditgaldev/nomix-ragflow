...
export function registerManagementTools(ctx: Context, client: RagFlowClient, transfer: TransferOptions): void {
  ...
  register(ctx, defineTool({
    ...
    parameters: { input: { oneOf: [
      actionInput('list', { datasetId: requiredString }),
  ...
  register(ctx, defineTool({
    ...
    description: 'Stream documents between the Harness workspace and RAGFlow REST. Existing files are never overwritten.',
    parameters: { input: { oneOf: [
      actionInput('upload', { datasetId: requiredString, sourcePath: requiredString, displayName: { type: 'string' } }),
      actionInput('download', { datasetId: requiredString, documentId: requiredString, destinationPath: requiredString }),
    ], required: true } }, output,
    async execute(args, exec) {
      const input = object(args.input); const datasetId = string(input, 'datasetId')
      switch (string(input, 'action')) {
        ...
        case 'download': return downloadWorkspaceDocument(ctx, client, transfer, datasetId, string(input, 'documentId'), string(input, 'destinationPath'), exec.signal) as Promise<JsonValue>
  ...
  register(ctx, defineTool({
    ...
    parameters: { input: { oneOf: [
      actionInput('list', { datasetId: requiredString, documentId: requiredString }),
  ...
  register(ctx, defineTool({
    ...
    parameters: { input: { oneOf: [
      actionInput('create', { kind: { type: 'string', enum: ['chat', 'agent'], required: true }, ownerId: requiredString }),
      actionInput('list', { kind: { type: 'string', enum: ['chat', 'agent'], required: true }, ownerId: requiredString }),
      ...
      actionInput('ask', { kind: { type: 'string', enum: ['chat', 'agent'], required: true }, ownerId: requiredString, sessionId: requiredString }),
    ], required: true } }, output,
    async execute(args, exec) {
  ...
  register(ctx, defineTool({
    ...
    parameters: { input: { oneOf: [
      ...
      actionInput('list_messages', { memoryId: requiredString }),
      ...
      actionInput('search_messages', { query: requiredString, memoryIds: requiredStringArray }),
      actionInput('recent_messages', { memoryIds: requiredStringArray }),
    ], required: true } }, output,
    async execute(args, exec) {
